...
with open('restart23.dat', 'r') as f:simulation.context.setState(openmm.XmlSerializer.deserialize(f.read()))
...
WHAMrstr = CustomCVForce('4.184*5*((w1+w2)/2*10-23)^2')
...
simulation.reporters.append(DCDSubsetReporter('trj23.dcd', 5000, indices_list,append=True))#
simulation.reporters.append(StateDataReporter('log23.txt', 5000, step=True, potentialEnergy=True, temperature=True, volume=True, speed=True, separator='	',append=True))#
