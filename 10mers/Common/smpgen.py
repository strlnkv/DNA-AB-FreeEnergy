...
#starting from 24A, generate samples with different major groove widths
WHAMrstr = CustomCVForce('4.184*5*((w1+w2)/2*10-24)^2')
...
simulation.reporters.append(DCDSubsetReporter('24/trj24.dcd', 5000, indices_list))
simulation.reporters.append(StateDataReporter('24/log24.txt', 5000, step=True, potentialEnergy=True, temperature=True, volume=True, speed=True, separator='	'))
simulation.step(2500000)
state = simulation.context.getState( getPositions=True, getVelocities=True )
with open('24/restart24.dat', 'w') as f:f.write(openmm.XmlSerializer.serialize(state))
...
for i in range(16):
	WHAMrstr = CustomCVForce(f'4.184*5*((w1+w2)/2*10-{23-i})^2')
	w1=CustomBondForce('r')
	w1.addBond(28,372)
	w1.setUsesPeriodicBoundaryConditions(True)
	w2=CustomBondForce('r')
	...
	integrator = LangevinMiddleIntegrator(300*kelvin, 1/picosecond, 0.002*picoseconds)
	simulation = Simulation(chr.topology, system, integrator, platform)
	with open(f'{24-i}/restart{24-i}.dat', 'r') as f:simulation.context.setState(openmm.XmlSerializer.deserialize(f.read()))
	simulation.reporters.append(DCDSubsetReporter(f'{23-i}/trj{23-i}.dcd', 5000, indices_list))
	simulation.reporters.append(StateDataReporter(f'{23-i}/log{23-i}.txt', 5000, step=True, potentialEnergy=True, temperature=True, volume=True, speed=True, separator='	'))
	simulation.step(2500000)
	state = simulation.context.getState( getPositions=True, getVelocities=True )
	with open(f'{23-i}/restart{23-i}.dat', 'w') as f:f.write(openmm.XmlSerializer.serialize(state))
	system.removeForce(whamrstrid)
	del simulation
	del integrator
	del WHAMrstr
