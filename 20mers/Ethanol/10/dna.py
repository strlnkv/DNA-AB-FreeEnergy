...
WHAMrstr = CustomCVForce('4.184*5*((w1+w2+w3+w4+w5+w6+w7+w8+w9+w10+w11+w12)/12*10-10)^2')
...
simulation.reporters.append(DCDSubsetReporter('trj10.dcd', 5000, indices_list,append=True))#
simulation.reporters.append(StateDataReporter('log10.txt', 5000, step=True, potentialEnergy=True, temperature=True, volume=True, speed=True, separator='	',append=True))#
simulation.runForClockTime(1.4)
state = simulation.context.getState( getPositions=True, getVelocities=True )
with open('restart10.dat', 'w') as f:f.write(openmm.XmlSerializer.serialize(state))
